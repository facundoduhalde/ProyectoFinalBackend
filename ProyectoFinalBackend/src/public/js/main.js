...
const createMessageUsuario = async (message) => {
	const mensajeNuevo = new MensajeModel({
		userId: userId,
		tipo: "Usuario",
		mensaje: message,
	});
	const creado = await mensajeNuevo.save();
};
const msgStandard = [
	'No he podido comprender tu mensaje. Por favor ingresa una de las siguientes opciones ****"Stock"**** : Para conocer nuestro stock actual \n ****"Orden"****: Para conocer la información de tu ultima orden ****"Carrito"****: Para conocer el estado actual de tu carrito',
];
...
function socketFunction(io) {
	io.on("connection", (socket) => {
		...
		socket.on("token", (message) => {
			...
			console.log("Token del cliente", tokenInformado);
		});
		socket.on("new-message", async (message) => {
			...
			if (!userIdOK) {
				...
				socket.emit(
					"resp-message",
					`UserId incorrecto, favor ingresarlo correctamente para continuar.`
				);
				return;
			}
			...
			if (message.includes("carrito")) {
				...
				socket.emit(
					"resp-message",
					`Stock de productos :   ${JSON.stringify(productos)}`
				);
				return;
			}
